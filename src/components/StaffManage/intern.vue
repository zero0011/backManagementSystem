        ...
        if (res.state === 1000) {
          ...
          for (let index in this.orgs) {
            this.orgs[index].details.push({
              'realName': '总计',
              'accountName': '实习: ' + this.orgs[index].summary.intern + '(人)'
            })
          }
        } else {
          setTimeout(() => {
...
.internMemberManageApp > main .el-table tbody tr:last-of-type {
  background-color: #e4e7ed;
}

.internMemberManageApp > main .el-table tbody tr:last-of-type:hover > td {
  background-color: #e4e7ed;
}
.internMemberManageApp
  > main
