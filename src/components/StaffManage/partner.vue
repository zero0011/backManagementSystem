        ...
        if (res.state === 1000) {
          ...
          for (let index in this.orgs) {
            this.orgs[index].details.push({
              'realName': '总计',
              'accountName': '合作: ' + this.orgs[index].summary.partner + '(人)'
            })
          }
        } else {
          setTimeout(() => {
...
.partnerMemberManageApp > main .el-table tbody tr:last-of-type {
  background-color: #e4e7ed;
}

.partnerMemberManageApp > main .el-table tbody tr:last-of-type:hover > td {
  background-color: #e4e7ed;
}
.partnerMemberManageApp
  > main
